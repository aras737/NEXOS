...
from core.economy import add_money, format_money
from core.embeds import make_embed
from core.logging import log_event


def register(bot):
    @bot.tree.command(name="eco-add", description="Yetkili olarak uyeye ekonomi kredisi ekler.")
    ...
    async def eco_add(
        ...
        account = add_money(interaction.guild.id, member.id, amount, target.value)
        embed = make_embed("Ekonomi Guncellendi", f"{member.mention} hesabina {format_money(amount)} eklendi.", 0x2ECC71)
        ...
        await log_event(
            interaction.guild,
            "Eco Add",
            f"{member} hesabina {format_money(amount)} eklendi.",
            0x2ECC71,
            [
